...
class ThetaCalculator:
    ...
    def calc_theta(self, T: np.ndarray, theta_base = 0):
        ...
        theta_1s = self.calc_theta_1(theta_base)
        for theta_1 in theta_1s:
            theta_2s = self.calc_theta_2(theta_1)
                        ...
                        for theta_4 in theta_4s:
                            ...
                            for theta_3 in theta_3s:
                                ...
                                for theta_5 in theta_5s:
                                    self.results.append([theta_1, theta_2, theta_3, theta_4, theta_5, theta_6, theta_7])
        
        return np.array(self.results)
               
    def calc_theta_1(self, theta_0):
        """
    ...
    def calc_theta_2(self, theta_1):
        ...
        Phi = np.arctan2(b, a)
        phi = np.arcsin(-0.3/np.sqrt(a**2 + b**2))
            
        return [phi - Phi, np.pi - phi - Phi]
    ...
    def calc_theta_6(self, theta_1, theta_2):
        ...
        return [np.arcsin(s6), -np.arcsin(s6)]
    ...
    def calc_theta_4(self, theta_1, theta_2, theta_7):
        ...
        return [np.arccos(c4), -np.arccos(c4)]
    ...
    def calc_theta_5(self, theta_1, theta_3, theta_4, theta_6, theta_7):
        ...
        return [theta_4 + theta_3 - np.arctan2(s345, c345)]
